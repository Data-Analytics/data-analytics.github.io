...
data_total = pd.read_csv('assembly_pondi.csv')

data_total = data_total.groupby(['year'])['polled_votes'].sum()

data['party'] = [j if j in ['DMK','IND','INC','CPM','CPI','ADMK','JNP','PMK','TMC(M)','BJP','MDMK','CPI(M)','DMDK','MNMK','AINRC'] else 'Others' for j in data['PARTY']]
...
data.to_csv('vote_share_predict_pondi.csv')
