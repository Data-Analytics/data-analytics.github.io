...
function merge_charts(data,width_value){
    ...
    var svg = d3.select("#"+animal+"_chart_combined").append("svg")
                    .attr("width", "100%")
                    .attr("data-height","0.54")
                    .attr("viewBox","0 0 "+(width + margin.left + margin.right)+" "+(height + margin.top + margin.bottom))
                    .attr("xmlns", 'http://www.w3.org/2000/svg')
    ...
    // group that will contain all of the plots
    var groups = svg.append("g").attr("transform", "translate(-20," + margin.top + ")");
    }
    ...
    else
    {
     svg.append("g")
        .attr("class", "x axis")
        .attr("transform", "translate(-20," + (height - 60 + margin.top) + ")")
        .call(xAxis);

     svg.append("text")
        .attr("class", "x label")
        .attr("text-anchor", "end")
        .attr("x", width + 50 - margin.left)
        .attr("y", height - margin.top - 5)
        .text(animal);
    }   
